...
def get_content_aware_pruning_scores(model, args, device):
    ...
    for (idx, batch) in enumerate(batch_size_list):
        ...
        c = torch.empty(batch, args.c_dim, device = device)
        img_tensor = model(*[z,c])
        ...
        grad_score = get_weight_gradient(noisy_img_tensor, img_tensor, model)
